...
class NeuronState {
  ...
  invoke() async {
    ...
    add(Neuron(const Uuid().v4(), const Uuid().v4(), Note("Ein etwas längerer Body einer Notiz, der auch gerne über mehrere Zeilen gehen darf, um zu testen, wie das ausschaut :)", "note", "Neue Notiz", 3), DateTime.now(), [], [tagWichtig]));
    add(Neuron(const Uuid().v4(), const Uuid().v4(), Date("Neuer Termin", DateTime.utc(2023, 07, 15), "date", "Prüfungsdatum Mathe", 4), DateTime.now(), [], [tagUni, tagWichtig, tagUni, tagWichtig]));
    add(Neuron(const Uuid().v4(), const Uuid().v4(), Task("", false, DateTime.now().add(const Duration(seconds: 30)), "task", "Geschirr", 1), DateTime.now(), [], [tagHaushalt]));
    add(Neuron(const Uuid().v4(), const Uuid().v4(), Note("Ein Body einer Notiz:)", "note", "Neue Notiz", 3), DateTime.now(), [], [tagArbeit]));
